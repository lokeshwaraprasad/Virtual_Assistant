...
#set_api_key("sk_edcbf4c4f8234d322c765cc5b6cf66cb23be8e5202f69e0d")
...
#def engine_talk(query):
  ...
  #  )
     #play(audio)
from elevenlabs import ElevenLabs, VoiceSettings, play, save, voices
from elevenlabs.client import ElevenLabs

# Replace with your actual API key
client = ElevenLabs(
    api_key="sk_edcbf4c4f8234d322c765cc5b6cf66cb23be8e5202f69e0d"
)
...
for voice in available_voices.voices:
    print(f"Name: {voice.name}, ID: {voice.voice_id}, Gender: {voice.labels.get('gender', 'Unknown')}")
...
def initialize_engine():
    ...
    male_voice = None
    for voice in voices:
        if "male" in voice.name.lower() or "david" in voice.name.lower():  # Common male voice names
            male_voice = voice
            break

    #engine.setProperty('voice', voices[1].id)
    engine.setProperty('voice', male_voice.id if male_voice else voices[0].id)
...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good morning Lokesh, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon Lokesh, it's {day} and the time is {t}")
    else:
        speak(f"Good evening Lokesh, it's {day} and the time is {t}")
...
#def closeApp(command):
      ...
      #os.system('powershell "Get-Process -Name CalculatorApp | Stop-Process -Force"')

   # elif "notepad" in command:
     # speak("Closing the notepad")
      #os.system('taskkill/f /im notepad.exe')
     # os.system('powershell "Get-Process -Name notepad | Stop-Process -Force"')


    #elif "paint" in command:
      #speak("closing the Paint")
...
def closeApp(command):
    try:
        ...
        speak(f"Sorry, I couldn't close the application. Error: {str(e)}")
